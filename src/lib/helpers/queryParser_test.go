...
func TestMaxLimitEmptyString(t *testing.T) {
	emptyLimit := MaxLimit("", 1, 20)

	if emptyLimit != 1 {
		t.Errorf("MaxLimit(\"\", 1, 20) failed. Expected %v, Received %v", 1, emptyLimit)
	} else {
...
func TestMaxLimitBadString(t *testing.T) {
	limit := MaxLimit("test", 1, 20)

	if limit != 1 {
		t.Errorf("MaxLimit(\"test\", 1, 20) failed. Expected %v, Received %v", 1, limit)
	} else {
...
func TestMaxLimitOverMaxString(t *testing.T) {
	limit := MaxLimit("300", 1, 20)

	if limit != 20 {
		t.Errorf("MaxLimit(\"300\", 1, 20) failed. Expected %v, Received %v", 20, limit)
	} else {
